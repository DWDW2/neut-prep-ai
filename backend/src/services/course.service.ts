...
export default class CourseService {
  async generateLessonMath(roadmapId: string, lessonIndex: number, unitIndex: number) {
    try {
      const roadmap = await RoadMap.findById(roadmapId);
      if (!roadmap) return null;
      const lesson = roadmap.roadmap[unitIndex].lessons[lessonIndex]
      ...
      if(typeof lessonModelInstance.id === 'string'){
          roadmap.roadmap[unitIndex].lessons[lessonIndex].lessonContent = lessonModelInstance.id
      }
  ...
  async generateLessonCritical(roadmapId:string, unitIndex: number, lessonIndex: number){
    try {
      const roadmap = await RoadMap.findById(roadmapId);
    if (!roadmap) return null;
    const lesson = roadmap.roadmap[unitIndex].lessons[lessonIndex]
